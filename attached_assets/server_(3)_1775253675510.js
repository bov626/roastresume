...
function startPolling(submissionId) {
  ...
  const intervalId = setInterval(async () => {
    ...
    if (ready) {
      ...
      const emailOk = await sendRoastEmail(row.email, firstName, row.watch_token);
...
app.post('/api/submit', resumeUpload.single('resume'), async (req, res) => {
  ...
  try {
    await fetch(`https://api.beehiiv.com/v2/publications/${process.env.BEEHIIV_PUBLICATION_ID}/subscriptions`, {
      ...
      body: JSON.stringify({ email, first_name: firstName, last_name: lastName, reactivate_existing: true, send_welcome_email: true })
    });
...
app.get('/watch/:token', async (req, res) => {
  ...
  if (!row || !row.cloudflare_video_id) {
    return res.status(404).send(`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Not Ready</title>
...
app.post('/admin/login', (req, res) => {
  if (req.body.password === PASS) { req.session.authed = true; return res.redirect('/admin'); }
...
app.post('/admin/api/upload-video/:id', requireAuth, videoUpload.single('video'), async (req, res) => {
  ...
  try {
    ...
    const cfResult = await uploadToCloudflare(localPath, req.file.originalname);
    ...
    await supabase
      .from('submissions')
      .update({ cloudflare_video_id: videoId, status: 'done', email_sent_at: null })
      .eq('id', id);
    ...
    if (fs.existsSync(localPath)) fs.unlinkSync(localPath);
    res.status(500).json({ ok: false, error: 'Cloudflare upload failed: ' + err.message });
  }
});
...
<style>
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .eyebrow{font-size:10px;font-weight:800;letter-spacing:.2em;text-transform:uppercase;color:#d91e1e;margin-bottom:14px;opacity:.8}
  ...
  .cta-sub{font-size:13px;color:#444;line-height:1.75;margin-bottom:24px}
...
<body>
<nav class="nav">
  <div class="nav-logo">Please Roast My Resume</div>
  <a class="nav-link" href="https://jumpseatjobs.com" target="_blank">Want a second job? →</a>
</nav>
<div class="hero">
...
<div class="player-outer">
  <div class="player-inner">
    <iframe
      src="https://iframe.cloudflarestream.com/${videoId}?autoplay=false&letterboxColor=transparent&primaryColor=%23d91e1e"
...
<div class="cta">
  <div class="cta-eyebrow">What's next</div>
